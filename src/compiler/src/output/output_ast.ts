...
export class AstTransformer implements StatementVisitor, ExpressionVisitor {
  ...
  visitTypeofExpr(expr: TypeofExpr, context: any): any {
    return this.transformExpr(
      new TypeofExpr(
        expr.expr.visitExpression(this, context),
        expr.type,
        expr.sourceSpan
      ),
      context
  ...
  visitNotExpr(ast: NotExpr, context: any): any {
    return this.transformExpr(
      new NotExpr(ast.condition.visitExpression(this, context), ast.sourceSpan),
      context
    );
  }
  ...
  visitLiteralMapExpr(ast: LiteralMapExpr, context: any): any {
    ...
    return this.transformExpr(
      new LiteralMapExpr(entries, mapType, ast.sourceSpan),
      context
    );
  }
  visitCommaExpr(ast: CommaExpr, context: any): any {
  ...
  visitDeclareClassStmt(stmt: ClassStmt, context: any): any {
    ...
    const getters = stmt.getters.map(
      (getter) =>
        new ClassGetter(
          getter.name,
          this.visitAllStatements(getter.body, context),
          getter.type,
          getter.modifiers
        )
    );
    const ctorMethod =
      ...
      new ClassMethod(
        stmt.constructorMethod.name,
        stmt.constructorMethod.params,
        this.visitAllStatements(stmt.constructorMethod.body, context),
        stmt.constructorMethod.type,
        stmt.constructorMethod.modifiers
      );
    const methods = stmt.methods.map(
      (method) =>
        new ClassMethod(
          method.name,
          method.params,
          this.visitAllStatements(method.body, context),
          method.type,
          method.modifiers
        )
    );
...
class _ReadVarVisitor extends RecursiveAstVisitor {
  ...
  override visitDeclareFunctionStmt(
