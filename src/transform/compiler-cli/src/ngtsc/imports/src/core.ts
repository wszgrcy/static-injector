...
/**
 * A mapping of supported symbols that can be imported from within static-injector, and the names by
...
const CORE_MODULE = "static-injector";

/**
 * `ImportRewriter` that rewrites imports from 'static-injector' to be imported from the r3_symbols.ts
 * file instead.
 */
...
export function validateAndRewriteCoreSymbol(name: string): string {
  if (!CORE_SUPPORTED_SYMBOLS.has(name)) {
    throw new Error(
      `Importing unexpected symbol ${name} while compiling ${CORE_MODULE}`
    );
  }
