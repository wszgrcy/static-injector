...
export function extractClassMetadata(
  ...
  const ngClassDecorators = classDecorators
    ...
    .map((decorator) =>
      decoratorToMetadata(decorator, annotateForClosureCompiler)
    )
  ...
  const metaDecorators = new WrappedNodeExpr(
    ts.createArrayLiteral(ngClassDecorators)
  );
...
function decoratorToMetadata(
  ...
  if (decorator.args !== null && decorator.args.length > 0) {
    const args = decorator.args.map((arg) => {
      ...
      return wrapFunctionsInParens
        ? wrapFunctionExpressionsInParens(expr)
        : expr;
    });
    properties.push(
 ...
 *
 * Either it's used in static-injector, or it's imported from there.
 * todo 装饰器来源判断
 */
 ...
 */
function removeIdentifierReferences<T extends ts.Node>(
  node: T,
  name: string
): T {
  const result = ts.transform(node, [
    (context) => (root) =>
      ts.visitNode(root, function walk(current: ts.Node): ts.Node {
