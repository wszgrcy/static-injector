...
export class DynamicValue<R = unknown> {
  ...
  static fromInvalidExpressionType(
    node: ts.Node,
    value: unknown
  ...
  isFromDynamicInput(
    this: DynamicValue<R>
  ): this is DynamicValue<DynamicValue> {
  ...
  isFromExternalReference(
    this: DynamicValue<R>
  ): this is DynamicValue<Reference<ts.Declaration>> {
  ...
  accept<R>(visitor: DynamicValueVisitor<R>): R {
    switch (this.code) {
      case DynamicValueReason.DYNAMIC_INPUT:
        return visitor.visitDynamicInput(
          this as unknown as DynamicValue<DynamicValue>
        );
