...
const BINARY_OPERATORS = new Map<ts.SyntaxKind, BinaryOperatorDef>([
  ...
  [
    ...
    literalBinaryOp((a, b) => Math.pow(a, b)),
  ],
...
export class StaticInterpreter {
  constructor(
    ...
    private dependencyTracker: null
  ) {}
      ...
      if (ts.isPropertyAssignment(property)) {
        ...
        if (name === undefined) {
          return DynamicValue.fromDynamicInput(
            node,
            DynamicValue.fromDynamicString(property.name)
          );
        }
        ...
        if (symbol === undefined || symbol.valueDeclaration === undefined) {
          ...
          map.set(
            property.name.text,
            this.visitDeclaration(symbol.valueDeclaration, context)
          );
        }
  ...
  private visitIdentifier(
    ...
    if (decl.known !== null) {
      ...
      isConcreteDeclaration(decl) &&
      decl.identity !== null &&
      decl.identity.kind === SpecialDeclarationKind.DownleveledEnum
    ) {
      return this.getResolvedEnum(
        decl.node,
        decl.identity.enumMembers,
        context
      );
    }
    const declContext = {
  ...
  private visitEnumDeclaration(
    ...
    node.members.forEach((member) => {
      ...
      if (name !== undefined) {
        const resolved =
          member.initializer && this.visit(member.initializer, context);
  ...
  private visitAmbiguousDeclaration(decl: Declaration, declContext: Context) {
    return decl.kind === DeclarationKind.Inline &&
      decl.implementation !== undefined &&
  ...
  private accessHelper(
    ...
    if (lhs instanceof Map) {
      ...
      if (this.host.isClass(ref)) {
        ...
        return DynamicValue.fromDynamicInput(
          node,
          DynamicValue.fromExternalReference(
            ref,
            lhs as Reference<ts.Declaration>
          )
        );
      }
  ...
  private visitCallExpression(
    ...
    // value.
    if (
      res instanceof DynamicValue &&
      context.foreignFunctionResolver !== undefined
    ) {
  ...
  private getReference<T extends DeclarationNode>(
    node: T,
    context: Context
  ): Reference<T> {
...
function isVariableDeclarationDeclared(node: ts.VariableDeclaration): boolean {
  ...
  if (
    declList.parent === undefined ||
    !ts.isVariableStatement(declList.parent)
  ) {
    return false;
  }
...
function joinModuleContext(
  ...
  if (
    decl.viaModule !== null &&
    decl.viaModule !== existing.absoluteModuleName
  ) {
    return {
...
function owningModule(
  context: Context,
  override: OwningModule | null = null
): OwningModule | null {
