...
import { getInjectableDef, getInjectorDef, InjectorType, InjectorTypeWithProviders, ɵɵInjectableDeclaration } from './interface/defs';
...
export class R3Injector {
  ...
  get<T>(token: ProviderToken<T>, notFoundValue: any = THROW_IF_NOT_FOUND, flags = InjectFlags.Default): T {
    this.assertNotDestroyed();
  ...
  private processInjectorType(
  ...
  ): defOrWrappedDef is InjectorTypeWithProviders<any> {
    ...
    const ngModule = (def == null && (defOrWrappedDef as InjectorTypeWithProviders<any>).ngModule) || undefined;

    // Determine the InjectorType. In the case where `defOrWrappedDef` is an `InjectorType`,
    // then this is easy. In the case of an InjectorDefTypeWithProviders, then the definition type
    // is the `ngModule`.
    const defType: InjectorType<any> = ngModule === undefined ? (defOrWrappedDef as InjectorType<any>) : ngModule;
    ...
    return ngModule !== undefined && (defOrWrappedDef as InjectorTypeWithProviders<any>).providers !== undefined;
  }

  /**
   * Process a `SingleProvider` and add it.
   */
  private processProvider(provider: SingleProvider, ngModuleType: InjectorType<any>, providers: any[]): void {
...
function providerToRecord(provider: SingleProvider, ngModuleType: InjectorType<any>, providers: any[]): Record<any> {
